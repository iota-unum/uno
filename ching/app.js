  ...
  if (cast.hexArray.length === 6) {
    ...
    if(cast.counter>6){
        ...
        derivedBtn.addEventListener('click', e => {
             hex.getText(hex.derivedHexagram.binary)
                .then(data => {
        ...
        document.addEventListener("click", e => {
          ...
          if (
            ...
            const selectedHex = target.parentElement.parentElement.classList.contains('hexagram') ? hex.originalHexagram : hex.derivedHexagram
            
            console.log(selectedHex.data.hexagram, 'NUMBER');
            
    
              hex.getLine(selectedHex.data.number, num)
